		...
		if($err==1)
			...
			if($a=='addPost'){
				?><br><form action="<?=LINK?>adm/?r=created" enctype="multipart/form-data" method="post">
					...
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4add2menu">Добавити пост в меню сайту:</label>
						<div class="col-sm-9">
							<input type="checkbox" name="add2menu" id="a4add2menu" value="1" <?php if($row['add2menu']) echo 'checked';?>>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4add2mainp">Добавити пост на головну сторінку сайту:</label>
						<div class="col-sm-9">
				...
				?><br><form action="<?=LINK?>adm/?r=edited&id=<?=$id?>" enctype="multipart/form-data" class="overflowHiddenDivParrent" method="post">
					...
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4add2menu">Добавити в меню:</label>
						<div class="col-sm-9">
							<input type="checkbox" name="add2menu" id="a4add2menu" value="1" <?php if($row['add2menu'] || $_POST['add2menu']) echo 'checked';?>>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4add2mainp">Добавити на головну сторінку:</label>
						<div class="col-sm-9">
							<input type="checkbox" name="add2mainp" id="a4add2mainp" value="1" <?php if($row['add2mainp'] || $_POST['add2mainp']) echo 'checked';?>>
						</div>
					</div>
					<?php
			...
			echo '<div style="padding:10px;overflow:hidden">';#Статистика
				...
				if($_GET['r']=='created'){
					...
					if($u==0){
						...
						$b = array('&#60;','&#62;','&#34;','&#32;');
						$checked=($type==2)?1:0;
						$que='INSERT INTO '._POSTS_.' (link,name,content,date,creator_name,checked,unixTime,add2menu,add2mainp';
						...
						if($_POST['add2mainp']!=1)
							...
							$_POST['add2mainp']=1;
						$que.=') values("'.$path.'","'.$_POST['postName'].'","'.preg_replace($a,$b,nl2br($content,false)).'","'.$dt->format('d.m.Y в H:i').'","'.$row['id'].'","'.$checked.'","'.time().'","'.$_POST['add2menu'].'","'.$_POST['add2mainp'].'"';
						if(isset($_POST['postCategory']))
							$que.=',"'.$_POST['postCategory'].'"';
						$que.=');';
						que($que);
						echo $que.'<div class="alert alert-success">Пост успішно додано</div>';
					}
				}
				if($_GET['r']=='edited' && preg_match('/^[0-9]{1,}$/',$_GET['id'])){
					...
					$a = array('/</','/>/','/"/',"/'/");
					$b = array('&#60;','&#62;','&#34;','&#32;');
					$que='UPDATE '._POSTS_.' SET link="'.$path.'", name="'.$_POST['postName'].'", content="'.preg_replace($a,$b,nl2br($content,false)).'", edit_date="'.$dt->format('d.m.Y в H:i').'", editor_name="'.$g_id.'", checked="'.$checked.'", add2menu="'.$_POST['add2menu'].'", add2mainp="'.$_POST['add2mainp'].'"';
					if($mark!=0)
						$que.=', points="'.$mark.'"';
					if(isset($_POST['postCategory']))
				...
				que('SELECT * FROM '._POSTS_.' WHERE checked!=2');
				$u=mysql_num_rows($res);
				?>
