<?php
define('LINK','http://dumajdij.zzz.com.ua/');
define('_DOMAIN_','dumajdij.zzz.com.ua');
define('_DEAFULT_AVA_','https://s8.hostingkartinok.com/uploads/images/2018/01/74c8325ebdff4081319b7ca7adf4e7e8.png');
define('_LOCATION_',"http://dumajdij.zzz.com.ua/");
...
selCon2('about',2);
$about=$row['value'];
selCon2('email','info@dumajdij.zzz.com.ua');
selCon2('tel1');
selCon2('tel2');
selCon2('address');
selCon2('fblink');
...
function head ($style=0, $cont=0, $title = 'Головна',$others=0){
	...
	if($title!=NULL)
		$title.=' - ';
	if($description!=0)
		echo '<meta name="description" content="'.$title.$about.'">';
	else
		echo '<meta name="description" content="'.$description.'">';
	$keywords.=$tags;
	$title.='Комунікативний клуб "3D" для підлітків і батьків';
		echo '<link rel="shortcut icon" href="'.LINK.'favicon.ico" type="image/x-icon">
...
function socialML(){
	...
	if(_TWLINK_!=NULL) echo '<a target="_blank" href="'._TWLINK_.'"><img style="width: 30px;border-radius: 10px;" src="http://s8.hostingkartinok.com/uploads/images/2016/11/3f380a9e60a4bf1a8cf4e71ced861bc2.png"></a>';
		...
		<div class="container">
			...
			<div class="navbar-collapse collapse">
				<ul class="nav navbar-nav">	
					<? que('SELECT link,name FROM '._POSTS_.' WHERE add2menu="1"');
					while($row=mysql_fetch_array($res))
						echo '<li><a href="'.LINK.'p/'.$row['link'].'/">'.$row['name'].'</a></li>'; ?>
				</ul>
				<ul class="nav navbar-nav navbar-right">
					<?php $res = mysql_query('SELECT type,id,login FROM '._USERS_.' WHERE hesh="'.preg_replace('/[^a-z0-9]/','',$_COOKIE['hesh']).'"');
...
function down(){
	...
	?></div>
	<footer>
		<div class="container-fluid text-center down" style="background: #333;padding: 1% 13%;margin-top: 10px;"><?php
			if(isset($about)) echo '<div><h4>Про нас</h4><p>'.$about.'</p></div>';
			echo '<div>
				<h3>Контакти</h3>';
				if(_TEL1_!=NULL) echo '<p>'._TEL1_.'</p>';
				if(_TEL2_!=NULL) echo '<p>'._TEL2_.'</p>';
				echo '<p>'._EMAIL_.'</p><a class="aw" href="mailto:'._EMAIL_.'">Написати повідомлення</a>';
				if(_ADDRESS_!=NULL) echo '<p>'._ADDRESS_.'</p>';
				if(isset($keywords)) echo '<p style="position:absolute;bottom:0;width:0.1px;height:0.1px;overflow:hidden;">'.$keywords.'</p>';
			echo '</div><div>';
			socialML();
			?></div>
		</div>
		<div class="l_down text-center">&copy; <?=date("Y")?> Комунікативний клуб "3D" для підлітків та їх батьків</div>
	</footer>
	<script>
		$(document).ready(function() {
			var footerTop = $('footer').position().top + $('footer').height();
			if (footerTop < $(window).height()) {
				$('footer').css('margin-top', 10+ ($(window).height() - footerTop) + 'px');
			}
		});
	</script><?php
}
