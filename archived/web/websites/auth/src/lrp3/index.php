...
<body>
	...
	final class Site {
		...
		public static $tables = array(
			...
			array('dsproducts', 'h_simple_products', 'simple_products'),
		);
	...
	class Shop {
		...
		public static function shop_products() {
			global $sql;
			...
			if(array_key_exists('action',$_GET)){
				...
				if($_GET['action']=='add_product'){ ?>
					<form action="?ready=product_added" method="post">
						<input type="text" class="form-control" placeholder="Name" name="product_name"><br>
						<input type="number" class="form-control" placeholder="Price" name="product_price"><br>
						<textarea type="text" class="form-control" placeholder="Description" name="product_description"></textarea><br>
						<div class="btn-group">
				...
				if($_GET['action']=='edit_product' && array_key_exists('product_id',$_GET) && is_numeric($_GET['product_id']) && $_GET['product_id'] > 0){
					$product = $sql->r('SELECT * FROM dsproducts WHERE id = ?',$_GET['product_id']); ?>
					<form action="?ready=edit_product&product_id=<?=$_GET['product_id']?>" method="post">
						<input type="text" class="form-control" placeholder="Name" name="product_name" value="<?=$product['name']?>"><br>
						<input type="number" class="form-control" placeholder="Price" name="product_price" value="<?=$product['cost']?>"><br>
						<textarea type="text" class="form-control" placeholder="Description" name="product_description"><?=$product['content']?></textarea><br>
						<div class="btn-group">
							<input type="submit" class="btn btn-outline-dark" value="Edit product" name="submit">
							<a href="index.php" class="btn btn-outline-dark">To home</a>
							<a href="index.php?ready=delete_product&product_id=<?=$_GET['product_id']?>" class="btn btn-outline-dark">Delete product</a>
						</div>
					</form>
					<?php
				}

				if($_GET['action']=='show_product' && array_key_exists('product_id',$_GET) && is_numeric($_GET['product_id']) && $_GET['product_id'] > 0){ ?>
					<a href="index.php" class="btn btn-outline-dark">To home</a> <?php
					if($is_logiined){ ?>
						<a href="index.php?action=edit_product&product_id=<?=$_GET['product_id']?>" class="btn btn-outline-dark">Edit product</a> <?php
					}

					$product = $sql->r('SELECT * FROM dsproducts WHERE id = ?',$_GET['product_id']); ?>
					<div class="product">
						<h1 class="name"><?=$product['name']?></h1>
						<div class="date">Date: <?=date('d.m Y',$product['date'])?></div>
				...
				if($is_logiined){ ?>
					...
					if(array_key_exists('ready',$_GET)){
						if($_GET['ready'] == 'product_added' && array_key_exists('submit',$_POST)){
							$sql->a('INSERT INTO dsproducts(name,price,date,content) VALUES(?,?,?,?)',$_POST['product_name'],$_POST['product_price'],$_SERVER['REQUEST_TIME'],$_POST['product_description']);
						...
						if($_GET['ready']=='edit_product' && array_key_exists('submit',$_POST) && array_key_exists('product_id',$_GET) && is_numeric($_GET['product_id']) && $_GET['product_id'] > 0){
							$sql->a('UPDATE dsproducts SET name = ?, price = ?, date = ?, content = ? WHERE id = ?',$_POST['product_name'],$_POST['product_price'],$_SERVER['REQUEST_TIME'],$_POST['product_description'],$_GET['product_id']);
						...
						if($_GET['ready']=='delete_product' && array_key_exists('product_id',$_GET) && is_numeric($_GET['product_id']) && $_GET['product_id'] > 0){
							$sql->a('DELETE FROM dsproducts WHERE id = ?',$_GET['product_id']);
							Helper::alert('Product was successfully deleted!','success');
						}
					}
				}
				$products = $sql->res2data($sql->a('SELECT * FROM dsproducts'));
				foreach($products as $product){?>
					<div class="product">
						<a href="index.php?action=show_product&product_id=<?=$product['id']?>" class="name"><?=$product['name']?></a>
						<div class="date">Date: <?=date('d.m Y',$product['date'])?></div>
						<div class="price">Price: <?=$product['price']?> USD</div>
					</div>
					<?php
