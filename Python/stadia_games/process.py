...
money_spend_on_not_liked_games = sum([game["price"] for game in data['games'] if game['liked']=='not_liked'])
list_of_non_liked_purchased_games = [game["name"] for game in data['games'] if game['liked']=='not_liked' and game['price']!=0]
not_liked_pro_games = sum([1 for game in data['games'] if game['liked']=='not_liked' and game['price']==0])
list_of_non_liked_pro_games = [game["name"] for game in data['games'] if game['liked']=='not_liked' and game['price']==0]
...
paid_games_breakdown = {game['price'] / (game['playtime'] / 3600):'{name} ({price}) / {playtime} = {cost_per_hour} ({status})'.format(
		...
		status = format_status(game['liked']),
...
print((colored('GENERAL:','white')+'\n' +
	...
	'\n' +
	colored('PAID GAMES BREAKDOWN:','white')+'\n' +
