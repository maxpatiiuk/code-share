...
function r($users="users", $location, $domain="mambo.in.ua", $mail=NULL, $email=NULL, $name=NULL){
	...
	if(!isset($_POST['login']) && !isset($_POST['pass']) && !isset($_POST['login']) && !isset($_POST['d_pass']) && !isset($_POST['email'])){
		...
		if(strlen($_POST['class'])<1 || $_POST['class']<=0 || $_POST['class']>=13){
			$d=1;
			echo '<div class="alert alert-danger">Неправмльно вказаний клас, в якому ви навчаєтеся</div>';
		}
		...
		if($d==0){
			...
			$is_loggined=1;
			que("INSERT INTO ".$users."(login,val,ip,r_date,email,hesh,class) VALUES ('".$login."','".$pas."','".$_POST['ip']."','".date('H:i:s d:m:y')."','".strtolower($_POST['email'])."','".$hesh."','".$_POST['class']."')");
		}
	}
	if($d==1){
		?>
		<br><form class="form-horizontal" method="post" action="<?=$location?>register/">
			...
			<div class="form-group">
				<label class="control-label col-sm-2" for="a2class">Клас:</label>
				<div class="col-sm-9">
					<select class="form-control" name="class" id="a2class" required>
							...
							if(isset($_POST['submit_1'])){
								...
								if($_FILES['avatar']['size']>0){
									switch($_FILES['avatar']['type']){
										case "image/jpeg":$src2="jpg";break;
