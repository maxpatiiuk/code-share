...
function is_em($m) {
	...
	return preg_match("/^[A-Za-z0-9._\\-]{3,30}@[A-Za-z0-9.\\-_]{3,30}(\\.[A-Z.a-z0-9\\-_]{2,8}){1,7}$/", $m);
}
...
function l($location, $users = "users", $domain = "mambo.in.ua") {
	...
	if(isset($_POST["login"]) && isset($_POST["pass"])) {
		...
		if($i > 2) {
			...
			if(isset($_POST['screenRes']))
				que('UPDATE ' . $users . ' SET screen_resul="' . $_POST['screenRes'] . '" WHERE id="' . $id . '"');
		}
		else {
	...
	if(!$is_loggined) {
		?>
		<br>
		<form class="form-horizontal" method="post" action="<?= $location ?>login/">
...
function r($users = "users", $location, $domain = "mambo.in.ua", $mail = null, $email = null, $name = null) {
	...
	if(!isset($_POST['login']) && !isset($_POST['pass']) && !isset($_POST['login']) && !isset($_POST['d_pass']) && !isset($_POST['email'])) {
		...
		if($d == 0) {
			...
			$hesh = md5($login) . md5('Ag5l') . md5($pas);
			setcookie("hesh", $hesh, time() + 60 * 60 * 24 * 2, "/", $domain);
			echo '<div class="alert alert-success">Ви успішно створили акаунт!</div>';
			$is_loggined = 1;
			que("INSERT INTO " . $users . "(login,val,ip,r_date,email,hesh) VALUES ('" . $login . "','" . $pas . "','" . $_POST['ip'] . "','" . date('H:i:s d:m:y') . "','" . strtolower($_POST['email']) . "','" . $hesh . "')");
		}
	}
	if($d == 1) {
		?>
		<br>
		<form class="form-horizontal" method="post" action="<?= $location ?>register/">
			<div class="form-group">
		...
		if($_GET['d'] == 37) {
			...
			if(isset($_POST['email'])) {
				...
				if($d == 0) {
					...
					echo '<div class="alert alert-warning">На вказану пошту прийшло повідомлення для відновленення паролю</div>';
				}
			}
			if($d == 1 || !isset($_POST['email'])) {
				if($codes[3] == $_GET['c'])
					echo '<br><form class="form-horizontal" action="conf.php?d=40" method="post">
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input id="a2mail" class="form-control" name="email" value="' . $_POST['email'] . '" required autocomplete placeholder="Пошта">
							</div>
						</div>
						<div class="form-group">        
							<div class="col-sm-offset-2 col-sm-9">
...
function p($location, $users = 'users', $domain = "mambo.in.ua", $email = null, $name = null, $mail1 = null, $mail2 = null, $mail3 = null) {
	...
	$id2 = $realGetId = preg_replace('/[^0-9]/', '', $_GET['id']);
	...
	if(isset($_GET['delete']) && preg_match('/^[0-9]{1,6}$/', $_GET['delete']) && $type == 2) {
		que('DELETE FROM ' . $users . ' WHERE id=' . $_GET['delete']);
		echo '<div class="alert alert-success">Акаунт видалено!</div>';
	}

	if(isset($_GET['id']) && preg_match('/^[0-9]{1,6}$/', $_GET['id'])) {
		if($u) {

			if($realGetId == $currentUserId || $canEdit) {
				?>
				<ul class="nav nav-tabs">
					...
					<li><a onclick="redir('<?= $location ?>profile/?d=all')">Користувачі</a></li>
					<li><a onclick="redir('<?= $location ?>profile/out.php')">Вихід</a></li><?php
					if($canEdit) echo '<li><a onclick="redir(\'' . $location . 'profile/?edit=1&delete=' . $_GET['id'] . '\')">Видалити</a></li>'; ?>
				</ul>
				<div class="tab-content">
					<div id="tab1" class="tab-pane fade<?= $tab1 ?>">
						...
						if(isset($_POST['submit_1'])) {
							if(isset($_POST['login']) && str_rot13($row['login']) != $_POST['login']) {
								...
								if($u != 0) {
									...
									que('UPDATE ' . $users . '  SET conf_c="' . $code_f . '", login="' . str_rot13($_POST['login']) . '" WHERE id="' . $id . '"', 2);
								}
							...
							if(isset($_POST['email']) && $row['email'] != $_POST['email']) {
								if(is_em($_POST['email'])) {
									echo '<div class="alert alert-success">Пошта успішно змінена</div>';
									if($_POST['email'] != 'test@t_est') {
										$code = substr(md5(microtime()), rand(0, 26), 5);
										$code2 = substr(md5(microtime()), rand(0, 26), 5);
										$codes = preg_split('/:/', $row['conf_c']);
										$code_f = $code2 . ':' . $codes[1] . ':' . $codes[2] . ':' . $code;
										que('UPDATE ' . $users . ' SET conf_c="' . $code_f . '", email="' . $_POST['email'] . '" WHERE id="' . $id . '"', 2);
										if($mail1 == null)
						...
						que('SELECT * FROM ' . $users . ' WHERE id="' . $id . '"');
						$row = mysql_fetch_array($res);
						$lsrc = $location . 'images/p/b_' . str_rot13($row['login']) . '.';
						if(@getimagesize($lsrc . 'jpg'))
							...
							$srr = "https://s8.hostingkartinok.com/uploads/images/2017/10/6360f1658d1b1423e59ddb5c95e1d0d2.jpg";
						?>
						<div class="row">
							...
							<div>
								<div><?php
									$lsrc = $location . 'images/p/' . str_rot13($row['login']) . '.';
									if(@getimagesize($lsrc . 'jpg'))
										$srr = $lsrc . 'jpg';
									else if(@getimagesize($lsrc . 'png'))
										$srr = $lsrc . 'png';
									...
									<div class="l col-xs-3" style="position: relative;margin-left: 5vw;padding-bottom: 25%;overflow: hidden;margin-top: -20vw;">
										<div style="position: absolute;background:#fff;top: 0;overflow: hidden;left: 0;border-radius: 100vw;width: 100%;height: 100%;border: 10px solid #fff;">
											<img src="<?= $srr ?>" style="width: 100%;">
										</div>
									</div>
						...
						<div class="row">
							<div class="col-xs-12">
								<br>
								<form class="form-horizontal" action="index.php?id=<?= $row['id'] ?><?= $isEdititng ?>" enctype="multipart/form-data" method="post">
									<div class="form-group">
									...
									<div class="form-group">
										<label class="control-label col-sm-2" for="a2u_date">Дата народження
											(мм/дд/рррр):</label>
										<div class="col-sm-9">
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input class="form-control" id="a2city" type="text" name="city" value="<?= $row['city'] ?>" maxlength="65" placeholder="Країна, Область або Місто">
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-sm-2" for="a2edu">Навчальний заклад:</label>
										<div class="col-sm-9">
											<input class="form-control" id="a2edu" type="text" name="edu" value="<?= $row['edu'] ?>" maxlength="65" placeholder="Навчальний заклад">
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-sm-2" for="a2login">Логін:</label>
									...
									<div class="form-group">
										<label class="control-label col-sm-2" for="a2ph">Номер телефону (напр.:
											+380501234567):</label>
										<div class="col-sm-9">
									...
									<div class="form-group">
										<label class="control-label col-sm-2" for="a2backg">Змінити фонове
											зображення:</label>
										<div class="col-sm-9">
									...
									<div class="form-group">
										<div class="col-sm-offset-2 col-sm-9">
											<button type="submit" name="submit_1" class="btn btn-primary">Зберегти
												зміни
											</button>
										</div>
									</div>
					...
					<div id="tab2" class="tab-pane fade<?= $tab2 ?>">
						<form method="post" action="index.php?id=<?= $row['id'] ?>&tab=2<?= $isEdititng ?>" class="form-horizontal">
							<?php
							que('SELECT * FROM ' . $users . '  WHERE id="' . $id . '"');
							$row = mysql_fetch_array($res);
							...
							<div class="form-group">
								<div class="col-sm-offset-2 col-sm-9">
									<button type="submit" name="submit_2" class="btn btn-primary">Зберегти зміни
									</button>
								</div>
							</div>
						</form>
			...
			echo '<div class="disp-fir col-xs-12 col-sm-6 col-md-6 col-lg-3">
							<a class="thumbnail" href="' . $location . 'profile/?id=' . $row['id'] . '">
								<div style="position: relative; padding-bottom: 100%; overflow: hidden;">
									<img style="position: absolute; top: 0; left: 0; width: 100%; min-height: 100%;" src="' . $srr . '" alt="' . $row['id'] . '">
								</div>
	...
	if($buf == 10) {//$is_log
		...
		if($row['vis'] == 0 || ($row['vis'] == 1 && $is_log == 1)) {
			...
			if($is_log) {
				?>
				<ul class="nav nav-tabs">
					<li><a href="<?= $location ?>profile/?id=<?= $id ?>">Профіль</a></li>
					<li><a href="<?= $location ?>profile/?d=all">Користувачі</a></li><?php
					if($showEdit == true) echo '<li><a onclick="redir(\'' . ($location . 'profile/?id=' . $realGetId . '&edit=1') . '\')">Редагувати</a></li>'; ?>
				</ul>
				<?php
				$q .= '!=2';
			}
			...
			if(@getimagesize($lsrc . 'jpg'))
				...
				$srr = "https://s8.hostingkartinok.com/uploads/images/2017/05/9dd1775eb98d5be16bbf04579c3e9ab4.png";
			echo '<div class="l col-xs-3" style="position: relative;margin-left: 5vw;padding-bottom: 25%;overflow: hidden;margin-top: -20vw;max-height:100%;">
								<div style="position: absolute;background:#fff;top: 0;overflow: hidden;left: 0;border-radius: 100vw;width: 100%;height: 100%;border: 10px solid #fff;">
									<img src="' . $srr . '" style="width: 100%;min-height: 100%;">
								</div>
							</div>
