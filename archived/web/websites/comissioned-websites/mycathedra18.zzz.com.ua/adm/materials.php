			...
			echo '<div style="padding:10px;overflow:hidden">';#Статистика
				...
				if($_GET['r']=='created'){
					...
					if($u==0){
						...
						$que='INSERT INTO '._POSTS_.' (link,name,content,date,creator_name,checked,class,unixTime,type';
						...
						if($_POST['add2mainp']!=1)
							...
							$_POST['add2mainp']=1;
						$que.=',add2mainp) values("'.$path.'","'.$_POST['postName'].'","'.preg_replace($a,$b,nl2br($content,false)).'","'.$dt->format('d.m.Y в H:i').'","'.$row['id'].'","'.$checked.'",'.$_POST['postClass'].',"'.time().'","1"';
						if(isset($_POST['postCategory']))
				...
				if($_GET['r']=='edited' && preg_match('/^[0-9]{1,}$/',$_GET['id'])){
					...
					if($type!=2)
						...
						if($_POST['add2mainp']!=1)
							...
							$_POST['add2mainp']=1;
					$que='UPDATE '._POSTS_.' SET link="'.$path.'", name="'.$_POST['postName'].'", content="'.preg_replace($a,$b,nl2br($content,false)).'", edit_date="'.$dt->format('d.m.Y в H:i').'", editor_name="'.$g_id.'", coment="'.$_POST['postcoment'].'", checked="'.$checked.'", add2mainp='.$_POST['add2mainp'].', class="'.$_POST['postClass'].'"';
					if($mark!=0)
						$que.=', points="'.$mark.'"';
					if(isset($_POST['postCategory']))
