...
<!--						</div>-->
<!--					</div>-->
                    <input id="a2link" name="postLink" type="hidden" required value="<?=$row['link']?>">


					<div class="form-group">
						<label class="control-label col-sm-2" for="a3name">Назва матеріалу:</label>
						<div class="col-sm-9">
							<input id="a3name" class="form-control" name="postName" required value="<?=$row['name']?>" placeholder="Назва матеріал">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4category">Категорія:</label>
...
//					}
					...
					if($mark!=0)
						$que.=', points="'.$mark.'"';
					if(isset($_POST['postCategory']))
						$que.=', category="'.$_POST['postCategory'].'", type="1"';
					echo $que.' WHERE id="'.$_GET['id'].'"';
					que($que.' WHERE id="'.$_GET['id'].'"');
					echo '<div class="alert alert-success">Матеріал успішно змінено</div>';
				}
				que('SELECT * FROM '._POSTS_.' WHERE checked!=2 AND type!=0');
				$u=mysql_num_rows($res);
				?>
