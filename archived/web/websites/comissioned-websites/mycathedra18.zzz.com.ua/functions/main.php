<?php
define('LINK','http://mycathedra18.zzz.com.ua/');
define('_DOMAIN_','mycathedra18.zzz.com.ua');
define('_LOCATION_',"http://mycathedra18.zzz.com.ua/");
define('_API_','../api/lrp3.php');

define('_DEAFULT_AVA_','https://s8.hostingkartinok.com/uploads/images/2017/10/6360f1658d1b1423e59ddb5c95e1d0d2.jpg');
...
selCon('posts','c_posts');
selCon('mv','u');
selCon('users','c_users');
selCon('slider','c_slider');
...
function getName($userID){
	...
	$row=mysql_fetch_array($res);
	if(strlen($row['u_name']) && strlen($row['u_surname']))
		return $row['u_name'].' '.$row['u_surname'];
	else
		return str_rot13($row['login']);
}
function getNameLocal($login,$name,$surname){
...
selCon2('about',2);
$about=$row['value'];
selCon2('email','info@mycathedra18.zzz.com.ua');
selCon2('tel1');
selCon2('tel2');
selCon2('address');
selCon2('fblink');
...
function head ($style=0, $cont=0, $title = 'Головна',$others=0){
	if($title!=NULL)
		$title.=' - ';
	$title.='Геолуцьк - поринь у світ геграфії';
