...
function r($users='users', $location, $domain='mambo.in.ua', $mail=NULL, $email=NULL, $name=NULL){
	...
	if(!isset($_POST['login']) && !isset($_POST['pass']) && !isset($_POST['login']) && !isset($_POST['d_pass']) && !isset($_POST['email']))
		...
		if($d==0){
			...
			$code_f=$code.':::';
			setcookie("hesh", $hesh, time()+60*60*24*2, "/", $domain);
			alert('Ви успішно створили акаунт! Для продовження, підтвердіть вашу пошту, перейшовши за посиланням в листі, надісланому вам','success');
			que("INSERT INTO ".$users."(login,val,ip,r_date,email,hesh,conf_c) VALUES ('".$login."','".$pas."','".$_SERVER['REMOTE_ADDR']."','".date('H:i:s d:m:y')."','".$email."','".$hesh."','".$code_f."')");
			que('SELECT id FROM '.$users.' WHERE login="'.$login.'"');
			$row=mysql_fetch_array($res);
			$message='Дякуємо за реєстрацією в '.$name.' ('.$domain.'). Для підтвердження пошти, перейдіть по цьому посиланню - '.$location.'register/conf.php?i='.$row['id'].'&c='.$code;
			mail($_POST['email'],"Підтвердження пошти - Mambo Shop",$message,"Content-Type: text/html; charset='utf-8'","From: ".$email);
		}
	}
	if($d==1){
		?>
