...
<html>
	...
	<body>
		...
		if($err==1)
			...
			$a=$_GET['a'];
			if($a=='addPost'){
				?><br><form action="<?=LINK?>adm/materials.php?r=created" enctype="multipart/form-data" method="post">
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4link">Посилання на матеріал:</label>
						<div class="col-sm-9">
							<div class="input-group">
								<span class="input-group-addon"><?=LINK.'p/m/'?></span>
								<input id="a4link" name="postLink" type="text" class="form-control" required placeholder="Вкажіть посилання для цього матеріалу">
								<span class="input-group-addon">/</span>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4name">Назва матеріалу:</label>
						<div class="col-sm-9">
							<input id="a4name" class="form-control" name="postName" required placeholder="Назва матеріалу">
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4subcategory">Під категорія:</label>
						<div class="col-sm-9">
							<input id="a4subcategory" class="form-control" name="postSubcategory" value="<?=$row['subcategory']?>" placeholder="Під категорія">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4add2mainp">Добавити матеріал на головну сторінку сайту:</label>
						<div class="col-sm-9">
							<input type="checkbox" name="add2mainp" id="a4add2mainp" value="1">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2">Контент матеріалу:</label>
						<div class="col-sm-9">
					...
					<div class="btn-group">
						...
						<a style="color:#fff;" href="<?=LINK?>adm/materials.php" class="btn btn-danger">Скасувати</a>
						<button type="submit" class="btn btn-primary">Створити матеріал</button>
					</div>
				</form>
				<?php
			}
				...
				$cId=1;
				$isrc = '../p/m/'.$path.'/img_';
				$fsrc = LINK.'p/m/'.$path.'/fil_';
				$asrc = LINK.'p/m/'.$path.'/aud_';
				for($i=0;$i<=strlen($c);){
					if(substr($c,$i,7)=="\|/</p>")
						break;
					if(substr($c,$i,9)=='\|/<img="'){
						$src=substr($c,$i+9,strrpos(substr($c,$i+9,15),'"c="'));
						rename('../p/m/'.$path.'/'.$src,$isrc.$cId.substr($src,strrpos($src,'.')));
						$i+=9+strlen($src)+4;
						...
						rename('../p/m/'.$path.'/'.$src,$asrc.$cId.$type2);
						$i+=9+strlen($src)+5;
						...
						$finalContent.='<div class="addableContentDiv"><a href="'.LINK.'p/m/'.$path.'/'.$fil.'" target="_blank" download>Завантажити файл ('.$fil.')</a><input type="hidden" name="realFilId_'.$cId.'" value="'.$cId.'"><input type="hidden" name="fileName_'.$cId.'" value="'.$fil.'"><br>Виберіть файл:<br><input id="a2au" type="file" name="fil_'.$cId.'"><button onclick="$(this).closest(\'.addableContentDiv\').remove();" type="button" class="r btn btn-danger btn-xs">X</button><br><br><hr><br></div>';
				...
				if($type==2 && $row['creator_name']!=_ID_)
					echo '<br><div class="alert">Ви можете самостійно виправити матеріал учня, залишити коментарь в відповідне поле і відправити на перездачу або вказати оцінку та натиснути "Опублікувати"</div><br>';
				?><br><form action="<?=LINK?>adm/materials.php?r=edited&id=<?=$id?>" enctype="multipart/form-data" class="overflowHiddenDivParrent" method="post">
					<?php if($row['editor_name']!=_ID_ && $type!=2){
						if(strlen($row['coment'])>1 && $type!=2)
							echo '<div class="form-group">
									<label class="control-label col-sm-2" for="a2comment">Коментарь від вчителя до вашого матеріалу:</label>
									<div class="col-sm-9">
						...
						echo '<div class="form-group">
							<label class="control-label col-sm-2" for="a2coment">Коментарь учню до матеріалу:</label>
							<div class="col-sm-9">
					...
					<div class="form-group">
						<label class="control-label col-sm-2" for="a2link">Посилання на матеріал:</label>
						<div class="col-sm-9">
							<div class="input-group">
								<span class="input-group-addon"><?=LINK.'p/m/'?></span>
								<input id="a2link" name="postLink" type="text" class="form-control" required value="<?=$row['link']?>" placeholder="Вкажіть посилання для цього матеріалу">
								<span class="input-group-addon">/</span>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2" for="a3name">Назва матеріалу:</label>
						<div class="col-sm-9">
							<input id="a3name" class="form-control" name="postName" required value="<?=$row['name']?>" placeholder="Назва матеріалу">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4category">Категорія:</label>
					...
					<div class="form-group">
						<label class="control-label col-sm-2" for="a4add2mainp">Добавити матеріал на головну сторінку сайту:</label>
						<div class="col-sm-9">
							<input type="checkbox" name="add2mainp" id="a4add2mainp" value="1" <?php if($row['add2mainp']) echo 'checked';?>>
						</div>
					</div>
					<?php
					...
					<div class="btn-group">
						...
						<a style="color:#fff;" target="_blank" href="<?=LINK.'p/m/'.$row['link']?>/" class="btn btn-success">Переглянути матеріал</a>
						<a style="color:#fff;" href="<?=LINK?>adm/materials.php?r=deleted&id=<?=$id?>" class="btn btn-danger">Видалити матеріал</a>
						<a style="color:#fff;" href="<?=LINK?>adm/materials.php" class="btn btn-danger">Скасувати зміни</a><?php
			...
			echo '<div style="padding:10px;overflow:hidden">';#Статистика
				...
				echo '<div class="alert">На сайті зареєстровано '.$u.' користувачів<br>Загальна кількість матеріалів: '.$u1.'<br>Кількість матеріалів, які ви створили: '.$u2.'<br>Натисніть на рядок, щоб редагувати матеріал<br>Натисніть на клавішу "Додати матеріал" щоб додати новий матеріал</div>';
				if($_GET['r']=='deleted' && preg_match('/^[0-9]{1,}$/',$_GET['id'])){
					que('SELECT link FROM '._POSTS_.' WHERE id="'.$_GET['id'].'"');
					$row=mysql_fetch_array($res);
					array_map('unlink', glob('../p/m/'.$row['link'].'/*.*'));
					rmdir('../p/m/'.$row['link'].'/');
					que('DELETE FROM '._POSTS_.' WHERE link="'.$row['link'].'"');
					echo '<div class="alert alert-success">Матеріал успішно видалено</div>';
				}
				if($_GET['r']=='created'){
					...
					if($u==0){
						...
						if($clear){
							...
							$_POST['postLink'].=$buf;
							echo '<div class="alert alert-warning">Вказаний адрес матерілалу зайнятий. Систама автоматично вибрала рандомний адрес. Ви можете його змінити через меню редагування матерілалу</div>';
						}
						$lastContentId=$_POST['lastContentId'];
						$path=$_POST['postLink'];
						mkdir('../p/m/'.$path.'/');
						copy('../p/m/NO_DELETE/index.php','../p/m/'.$path.'/index.php');
						$content.='\\\\|/<p>';
						$cId=1;
						for($i=1;$i<=$lastContentId;$i++){
							if(isset($_POST['postContent_'.$i])){
								...
								if(preg_match('/image\//',$_FILES['img_'.$i]['type']) && $_FILES['img_'.$i]["size"]<10*1024*1024){
									move_uploaded_file($_FILES['img_'.$i]['tmp_name'],'../p/m/'.$path.'/img_'.$cId.'.'.pathinfo($_FILES['img_'.$i]['name'],PATHINFO_EXTENSION));
								...
								if(in_array($f,$a) && $_FILES['aud_'.$i]["size"]<10*1024*1024){
									move_uploaded_file($_FILES['aud_'.$i]['tmp_name'],'../p/m/'.$path.'/aud_'.$cId.'.'.pathinfo($_FILES['aud_'.$i]['name'],PATHINFO_EXTENSION));
								...
								if($_FILES['fil_'.$i]['size']>10*1024*1024)
									...
									move_uploaded_file($_FILES['fil_'.$i]['tmp_name'],'../p/m/'.$path.'/'.$f);
						...
						$b = array('&#60;','&#62;','&#34;','&#32;');
						$checked=($type==2)?1:0;
						$que='INSERT INTO '._POSTS_.' (link,name,content,date,creator_name,checked,unixTime,type';
						if(isset($_POST['postCategory']))
							$que.=',category';
						if(isset($_POST['postSubcategory']))
							$que.=',subcategory';
						if($_POST['add2mainp']!=1)
							$_POST['add2mainp']=0;
						else
							$_POST['add2mainp']=1;
						$que.=',add2mainp) values("'.$path.'","'.$_POST['postName'].'","'.preg_replace($a,$b,nl2br($content,false)).'","'.$dt->format('d.m.Y в H:i').'","'.$row['id'].'","'.$checked.'"';
						$que.=',"'.time().'","1"';
						if(isset($_POST['postCategory']))
							$que.=',"'.$_POST['postCategory'].'"';
						if(isset($_POST['postSubcategory']))
							$que.=',"'.$_POST['postSubcategory'].'"';
						$que.=','.$_POST['add2mainp'].');';
						que($que);
						echo '<div class="alert alert-success">Матеріал успішно додано</div>';
					}
				}
				if($_GET['r']=='edited' && preg_match('/^[0-9]{1,}$/',$_GET['id'])){
					...
					if($clear){
						...
						$_POST['postLink'].=$buf;
						echo '<div class="alert alert-warning">Вказаний адрес матеріалу зайнятий. Систама автоматично вибрала рандомний адрес. Ви можете його змінити через меню редагування матеріалу</div>';
					}
					$content.='\\\\|/<p>';
					$path=$_POST['postLink'];
					$psrc='../p/m/'.$path.'/img_';
					...
					que('SELECT link FROM '._POSTS_.' WHERE id="'.$_GET['id'].'"');
					$row=mysql_fetch_array($res);
					rename('../p/m/'.$row['link'],'../p/m/'.$_POST['postLink']);
					que('SELECT id FROM '._USERS_.' WHERE hesh="'.$_COOKIE['hesh'].'"');
					$row=mysql_fetch_array($res);
					...
					if($type!=2)
						...
						if($_POST['add2mainp']!=1)
							...
							$_POST['add2mainp']=1;
					$que='UPDATE '._POSTS_.' SET link="'.$path.'", name="'.$_POST['postName'].'", content="'.preg_replace($a,$b,nl2br($content,false)).'", edit_date="'.$dt->format('d.m.Y в H:i').'", editor_name="'.$g_id.'", coment="'.$_POST['postcoment'].'", checked="'.$checked.'", add2mainp='.$_POST['add2mainp'];
					if($mark!=0)
						$que.=', points="'.$mark.'"';
					if(isset($_POST['postCategory']))
						$que.=', category="'.$_POST['postCategory'].'"';
					if(isset($_POST['postSubcategory']))
						$que.=', subcategory="'.$_POST['postSubcategory'].'"';
					echo $que.', type="1" WHERE id="'.$_GET['id'].'"';
					que($que.', type="1" WHERE id="'.$_GET['id'].'"');
					echo '<div class="alert alert-success">Матеріал успішно змінено</div>';
				}
				que('SELECT * FROM '._POSTS_.' WHERE checked!=2 AND type=1');
				$u=mysql_num_rows($res);
				?>
				<table id="tableCond" class="table table-striped">
					...
					<tbody>
						...
						while($row=mysql_fetch_array($res)){
							...
							echo '<tr';
							if($type==2 || $row['creator_name']==$id)
								echo ' style="cursor: pointer" onclick="window.document.location=\''.LINK.'adm/materials.php?a=editPost&id='.$row['id'].'\'"';
							echo '>
				...
				<a href="'.LINK.'adm/materials.php?a=addPost" style="color:#fff" class="btn btn-success"><i class="glyphicon glyphicon-plus"></i> Додати матеріал</a>';
				if($type==2)
