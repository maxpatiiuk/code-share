...
<html>
	...
	<body>
		...
		elseif(isset($_GET['a'])){
				...
				?><br><form action="<?=LINK?>adm/?r=created" enctype="multipart/form-data" method="post"><!--
					<div class="form-group">
						...
						<div class="col-sm-9">
							...
							<p>Посилання не повине складатися з пробілів та спеціальних символи. Ви можете використовувати англійський алфавіт та цифри</p>
						</div>
					</div>-->
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							...
							<p>Приклади категорій: Aficiones, Deporte, Arte, Ciencias y tecnología, Cultur, Historia</p>
						</div>
					</div>
					<div class="form-group">
			...
			elseif($a=='editPost' && preg_match('/^[0-9]{1,}$/',$_GET['id'])){
				$id=$_GET['id'];
				function prr($res){
				...
				?><br><form action="<?=LINK?>adm/?r=edited&id=<?=$id?>" enctype="multipart/form-data" class="overflowHiddenDivParrent" method="post">
					...
					elseif($type==2 && $row['creator_name']!=_ID_){
						echo '<div class="form-group">
			...
			echo '<div style="padding:10px;overflow:hidden">';#Статистика
				...
				if($_GET['r']=='created'){
//					$_POST['postLink']=htmlspecialchars($_POST['postLink']);
					$_POST['postLink']=time();
					...
					if($u==0){
						...
						if($type==2)
							...
							que('SELECT email FROM '._USERS_.' WHERE type = 2 AND email NOT LIKE "test%" AND email NOT LIKE "roma.myster%" LIMIT 1');
							$raw = mysql_fetch_assoc($res);
							mail($raw['email'],'Nueva publicación en el sitio','Título: '.$_POST['postName']."\n".'Verificar: <a href="'.LINK.'/adm">'.LINK.'/adm</a>');
						}
...
//					}
					...
					for($i=1;$i<=$_POST['rLastContentId'];$i++,$was=0){
						...
						elseif(isset($_POST['imgAlign_'.$i])){
							$was=1;
							if($_FILES['img_'.$i]['size']>0){
								...
								if (@getimagesize($psrc.$i.'.jpg'))
									$src2='jpg';
								elseif (@getimagesize($psrc.$i.'.jpeg'))
									$src2='jpeg';
								elseif (@getimagesize($psrc.$i.'.png'))
									$src2='png';
								elseif (@getimagesize($psrc.$i.'.tiff'))
									$src2='tiff';
								elseif (@getimagesize($psrc.$i.'.gif'))
									$src2='gif';
						...
						elseif(isset($_POST['realAudId_'.$i])){
							$was=1;
							if($_FILES['aud_'.$i]['size']>0){
								...
								if(file_exists($asrc.$i.'.mp3'))
									$src3='.mp3';
								elseif(file_exists($asrc.$i.'.wav'))
									$src3='.wav';
								elseif(file_exists($asrc.$i.'.ogg'))
									$src3='.ogg';
						...
						elseif(isset($_POST['realFilId_'.$i])){
							$was=1;
							...
							elseif(isset($_POST['fileName_'.$i])){
...
//					rename('../p/'.$row['link'],'../p/'.$_POST['postLink']);
				...
				que('SELECT * FROM '._POSTS_.' WHERE checked!=2 && type=0 ORDER BY id+0 DESC');
				$u=mysql_num_rows($res);
				?>
