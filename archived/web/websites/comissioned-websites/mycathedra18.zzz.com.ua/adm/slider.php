...
<html>
	...
	<body>
		...
		<div style="padding:10px;overflow:hidden">
			...
			<a href="<?=LINK?>adm/materials.php" class="btn btn-success" style="color:#fff">Редагувати матеріали</a><?php
				...
				for($i=0;$i<$_POST['lastSlideId']+1;$i++){
					...
					if($_FILES['sImg_'.$i]['size']>0){
						switch($_FILES['sImg_'.$i]['type']){
							case "image/jpg":$src2="jpg";$t=1;break;
							case "image/jpeg":$src2="jpeg";$t=1;break;
					...
					<table class="table table-striped">
						...
						while($row=mysql_fetch_array($res)){
							if (@getimagesize($psrc.$row['id'].'.jpg'))
								$sliderSrc='jpg';
							else if (@getimagesize($psrc.$row['id'].'.jpeg'))
								$sliderSrc='jpeg';
